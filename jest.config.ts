...
const config: Config.InitialOptions = {
  ...
  moduleNameMapper: {
    ...
    // SVG base64 모듈 모의 처리
    'src/public/images/constants/svg-base64': '<rootDir>/src/__mocks__/svgBaseMock.js',
  ...
  collectCoverageFrom: [ // 커버리지 수집 파일 패턴
    'src/**/*.{ts,tsx}',
    '!src/**/*.d.ts',
    '!src/**/*.stories.{ts,tsx}',
    '!src/**/*.mock.{ts,tsx}',
  ...
  detectOpenHandles: false, // 열린 핸들 감지 (현재 테스트에서는 끄고 프로덕션에서 사용)
  forceExit: true, // 테스트 완료 후 강제 종료 (메모리 누수 문제 해결을 위해 임시로 true로 설정)
